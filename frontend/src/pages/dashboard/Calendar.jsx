...
export default function Calendar() {
  ...
  const buildSafeDate = (selectedMonth, selectedDay) => {
    if (!selectedMonth || !selectedDay) return '';

    const { year, month: currentMonth, day: currentDay } = getTodayParts();
    ...
    if (month === currentMonth && Number(day) < Number(currentDay)) {
      day = currentDay;
    }
  ...
  const renderAddEventControl = () => (
    <div className="relative mt-4 flex items-center gap-2 shrink-0">
      <button
      ...
      {showDropdown && (
        <div
          ref={dropdownRef}
          className="absolute left-0 top-full mt-2 p-3 bg-primary dark:bg-primary-dark border border-brd-primary dark:border-brd-primary-dark rounded shadow-lg z-10 w-72"
        >
          <input
            type="text"
          ...
          <div className="flex gap-2 mb-2">
            <select
              value={newEvent.date ? newEvent.date.split('-')[1] : ''}
              onChange={(e) => {
                const selectedMonth = e.target.value;
                const existingDay = newEvent.date ? newEvent.date.split('-')[2] : '';
                const { month: currentMonth, day: currentDay } = getTodayParts();
                ...
                const fallbackDay = existingDay || (selectedMonth === currentMonth ? currentDay : '01');
                const safeDate = buildSafeDate(selectedMonth, fallbackDay);

                setNewEvent(prev => ({ ...prev, date: safeDate }));
              }}
            ...
            <select
              value={newEvent.date ? newEvent.date.split('-')[2] : ''}
              onChange={(e) => {
                const selectedDay = e.target.value;
                const { month: currentMonth } = getTodayParts();
                const month = newEvent.date ? newEvent.date.split('-')[1] : currentMonth;
                ...
                const safeDate = buildSafeDate(month, selectedDay);
                setNewEvent(prev => ({ ...prev, date: safeDate }));
              }}
              className="flex-1 p-2 border border-brd-primary dark:border-brd-primary-dark rounded bg-secondary dark:bg-secondary-dark"
            >
          ...
          <button onClick={addEvent} className="w-full px-3 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
            Add
          </button>
        </div>
      )}
  ...
  const renderWeekView = () => {
    ...
    return (
      <div className="flex-1 min-h-0 rounded-lg border border-brd-primary dark:border-brd-primary-dark overflow-hidden w-full flex flex-col">
        {weekDays.map((d, i) => {
          ...
          return (
            <div
              key={i}
              className="flex-1 min-h-0 flex items-start gap-3 border-b border-brd-primary dark:border-brd-primary-dark last:border-b-0 px-3 py-2 bg-primary dark:bg-primary-dark"
            >
              ...
              <div className="flex-1 flex flex-wrap gap-1 items-start content-start overflow-y-auto">
                {dayEvents.length === 0 ? (
                  ...
                  dayEvents.map(event => (
                    <div
                      key={event.id}
                      className={`text-xs px-2 py-1 rounded bg-blue-500 text-white ${event.completed ? 'opacity-50' : ''}`}
                    >
                      {event.title}
                    </div>
                  ))
  ...
  const renderDayView = () => {
    ...
    return (
      <div className="flex-1 min-h-0 overflow-hidden">
        <div
          ref={dayViewRef}
          className="h-full rounded-lg border border-brd-primary dark:border-brd-primary-dark w-full overflow-y-auto"
          style={{
            scrollbarWidth: 'thin',
  ...
  return (
    <div className="p-4 md:p-2 bg-secondary dark:bg-primary-dark h-full min-h-0 overflow-hidden">
      <div className="flex flex-col h-full min-h-0 border-2 border-brd-primary dark:border-brd-primary-dark rounded-xl p-4 bg-primary dark:bg-primary-dark overflow-hidden">
